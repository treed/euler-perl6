# Solution in 17.3s
my @grid;
@grid[0] = <08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08>;
@grid[1] = <49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00>;
@grid[2] = <81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65>;
@grid[3] = <52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91>;
@grid[4] = <22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80>;
@grid[5] = <24 47 32 60 99 03 45 02 44 75 33 53 78 36 84 20 35 17 12 50>;
@grid[6] = <32 98 81 28 64 23 67 10 26 38 40 67 59 54 70 66 18 38 64 70>;
@grid[7] = <67 26 20 68 02 62 12 20 95 63 94 39 63 08 40 91 66 49 94 21>;
@grid[8] = <24 55 58 05 66 73 99 26 97 17 78 78 96 83 14 88 34 89 63 72>;
@grid[9] = <21 36 23 09 75 00 76 44 20 45 35 14 00 61 33 97 34 31 33 95>;
...
sub get_horizontal($x, $y) {
    gather for 0..3 -> $i {
        take @grid[$y][$x+$i];
    }
}

sub get_vertical($x, $y) {
    gather for 0..3 -> $i {
        take @grid[$y+$i][$x];
    }
}

sub get_nw_to_se($x, $y) {
    gather for 0..3 -> $i {
        take @grid[$y+$i][$x+$i];
    }
}

sub get_sw_to_ne($x, $y) {
    gather for 0..3 -> $i {
        take @grid[$y-$i][$x+$i];
    }
}
...
for 0..15 X 0..19 -> $x, $y {
    @products.push([*] get_horizontal($x, $y));
}

for 0..19 X 0..15 -> $x, $y {
    @products.push([*] get_vertical($x, $y));
}

for 0..15 X 0..15 -> $x, $y {
    @products.push([*] get_nw_to_se($x, $y));
}

for 0..15 X 3..19 -> $x, $y {
    @products.push([*] get_sw_to_ne($x, $y));
}
